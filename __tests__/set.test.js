 ...
 * 3. values() return undefined when empty; numeric size/array when non-empty
 * 4. size() returns 0 when empty; numeric size when non-empty
 * 5. remove() deletes existing; removing non-existent logs an error and does not change size
 * 6. clear() empties the set and resets size()/values()
 * 7. union() merges unique values from two non-empty sets
 * 8. intersection() returns only common values
 * 9. difference() returns values in first set not in the second
 * 10. symmetricDifference() returns values in either set but not both
 * 11. isSubSet/isSuperSet compute correct relations
 * 12. Methods validate instance type and log on invalid inputs
 */
...
describe("CustomSet", () => {
  ...
  test("4. size() returns 0 when empty; numeric size when non-empty", () => {
    const s = new CustomSet();
  ...
  test("4) remove() deletes existing and logs error when removing non-existent", () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Error: Item not found in the set"
    );
    expect(s.size()).toBe(1);
  ...
  test("11) methods validate instance type and log error message", () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Argument must be an instance of CustomSet"
    );
